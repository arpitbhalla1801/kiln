...
export async function loadPlugin(
  ...
  try {
    ...
    const capability = resolvePluginCapability(loadedModule);

    if (!capability) {
      return skip(
        entry,
        `'${entry.package}' does not export a usable Capability (expected a default export, or a 'capability' named export, implementing id/getManifest/getCapability/planAdd)`
      );
    }
...
function resolvePluginCapability(loadedModule: unknown): Capability | undefined {
  ...
  return candidates.find(isCapability) as Capability | undefined;
}
